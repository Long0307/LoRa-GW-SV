...
struct jit_node_s {
    ...
    struct timeval tx_timestamp;    /* Timestamp for transmission, based on host's UTC time */
...
struct jit_queue_s {
    uint8_t num_pkt;                /* Total number of packets in the queue (downlinks, ack...) */
...
@param time[in] Current host unix time
